...
def _is_mappable_value(value: Any) -> TypeGuard[Collection]:
    ...
    """
    return (
        not isinstance(value, (bytearray, bytes, str))
        if isinstance(value, (collections.abc.Sequence, dict))
        else False
    )
...
class TaskInstance(Base, LoggingMixin):
    ...
    def is_eligible_to_retry(self):
        ...
        if self.state == State.RESTARTING:
            ...
            return True
        return (
            self.task.retries and self.try_number <= self.max_tries
            if getattr(self, "task", None)
            else self.try_number <= self.max_tries
        )
