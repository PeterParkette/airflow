...
class S3Hook(AwsBaseHook):
    ...
    def _list_key_object_filter(
        ...
        def _is_in_period(input_date: datetime) -> bool:
            if from_datetime is not None and input_date <= from_datetime:
                return False
            return to_datetime is None or input_date <= to_datetime
    ...
    def get_wildcard_key(
        ...
        if key_matches := [
            k for k in key_list if fnmatch.fnmatch(k, wildcard_key)
        ]:
    ...
    def copy_object(
        ...
        return self.get_conn().copy_object(
    ...
    def delete_bucket(self, bucket_name: str, force_delete: bool = False) -> None:
        ...
        """
        if force_delete:
            if bucket_keys := self.list_keys(bucket_name=bucket_name):
    ...
    def download_file(
        ...
        if preserve_file_name:
            ...
            subdir = (
                f"airflow_tmp_dir_{uuid4().hex[:8]}"
                if use_autogenerated_subdir
                else ""
            )
