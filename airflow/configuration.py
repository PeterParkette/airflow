...
def _get_config_value_from_secret_backend(config_key: str) -> str | None:
    ...
    try:
        ...
        return secrets_client.get_config(config_key) if secrets_client else None
...
class AirflowConfigParser(ConfigParser):
    ...
    def _get_env_var_option(self, section: str, key: str):
        ...
        if (
            env_var_cmd in os.environ
            and (section, key) in self.sensitive_config_values
        ):
        ...
        if (
            env_var_secret_path in os.environ
            and (section, key) in self.sensitive_config_values
        ):
    ...
    def _get_option_from_config_file(
        ...
        if (
            deprecated_section
            and deprecated_key
            and super().has_option(deprecated_section, deprecated_key)
        ):
            if issue_warning:
                self._warn_deprecate(section, key, deprecated_section, deprecated_key, extra_stacklevel)
            with self.suppress_future_warnings():
    ...
    def as_dict(
        ...
        """
        if not display_sensitive and not all(
            [include_env, include_cmds, include_secret]
        ):
            raise ValueError(
    ...
    def _include_envs(
        ...
        for env_var in [
            ...
            if not display_sensitive and env_var != self._env_var_name("core", "unit_test_mode"):
                ...
                if (
                    not env_var.lower().endswith("cmd")
                    and not env_var.lower().endswith("secret")
                    and (section, key) in self.sensitive_config_values
                ):
                    opt = "< hidden >"
            elif raw:
    ...
    def _replace_section_config_with_display_sources(
        ...
        for k, val in items:
            ...
            if deprecated_section and deprecated_key and source_name == "default":
